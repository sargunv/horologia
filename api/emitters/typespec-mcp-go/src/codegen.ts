...
function emitGoCall(lines: string[], httpOp: HttpOperation, bodyFields: GoParam[]): void {
  ...
  if (opName === "list") {
    ...
    lines.push(`\t\t}`);
    lines.push(`\t\tif v, ok := args["limit"].(float64); ok && v >= 1 && v <= 100 {`);
    ...
    lines.push(`\t\ttitle, ok := args["title"].(string)`);
    lines.push(`\t\tif !ok || title == "" {`);
    ...
    lines.push(`\t\ttaskId, ok := args["taskId"].(string)`);
    lines.push(`\t\tif !ok || taskId == "" {`);
...
export function generateGoFile(program: Program, tools: ToolInfo[]): string {
  ...
  lines.push("// Regenerate: mise run generate  (from repo root)");
  lines.push("");
  ...
  lines.push("\t// Surface user-facing API errors (4xx) directly to the caller.");
  lines.push("\tvar apiErr *apigen.ErrorStatusCode");
  lines.push("\tif errors.As(err, &apiErr) {");
  lines.push("\t\treturn mcp.NewToolResultError(apiErr.Response.Error)");
  lines.push("\t}");
  lines.push(
