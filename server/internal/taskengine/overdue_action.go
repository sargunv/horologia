...
func applyOverdueAction(ctx context.Context, db database.DB, task dbgen.Task, now time.Time) error {
	...
	case dbgen.OverdueActionAdvanceRecurrence:
		return applyAdvanceRecurrence(ctx, db, task, now, nowTz)
	case dbgen.OverdueActionSetStatus:
		return applySetStatus(ctx, db, task, nowTz)
	case dbgen.OverdueActionClearDueDate:
		return applyClearDueDate(ctx, db, task, nowTz)
	default:
...
func applyAdvanceRecurrence(
	ctx context.Context, db database.DB, task dbgen.Task, now time.Time, nowTz pgtype.Timestamptz,
) error {
	q := dbgen.New(db)
...
func applySetStatus(
	ctx context.Context, db database.DB, task dbgen.Task, nowTz pgtype.Timestamptz,
) error {
